...
export const flipbookPageMap = new Map<string, FlipbookPage>(flipbookPages.map((page) => [page.id, page]));

export const totalFlipbookPages = flipbookPages.length;

export function getFirstIncompleteFlipbookPageId(completedPages: string[]): string {
  return flipbookPages.find((page) => !completedPages.includes(page.id))?.id ?? flipbookPages[0]?.id ?? "1";
}
